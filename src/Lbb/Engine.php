...
use Lbb\Interceptor\Dispatch as DispatchInterceptor;
use Lbb\SDK\SendCloudMailer;
use Eva\EvaSms\Sender;
use Phalcon\CLI\Console;
use Phalcon\Mvc\Router;
use Lbb\Mvc\Url as UrlResolver;
...
class Engine
{
    ...
    public function getDI()
    {
        ...
        $di->set('mailMessage', 'Lbb\MailMessage');
        ...
        $di->set('placeholder', 'Lbb\View\Helper\Placeholder');
            ...
            function () use ($di, $argv) {
                ...
                if ($moduleName) {
                    ...
                    if ($moduleName == '_current') {
                        ...
                        $dispatcher->setNamespaceName("Lbb\\{$moduleName}\\Tasks");
                    }
                } else {
                    $dispatcher->setTaskName('Main');
                    $dispatcher->setParams($argv);
                    $dispatcher->setNamespaceName("Lbb\\Tasks");
                }
    ...
    public function diRouter()
    {
        ...
        if ($moduleManager && $modulesArray = $moduleManager->getModules()) {
            foreach ($modulesArray as $moduleName => $module) {
                //NOTICE: LbbEngine Load front-end router at last
    ...
    protected function diCache($configKey, $prefix = 'eva_')
    {
        ...
        if (!$config->cache->enable || !$config->cache->$configKey->enable) {
            $cache = new \Lbb\Cache\Backend\Disable($frontCache);
        } else {
    ...
    public function diSession()
    {
        $adapterMapping = array(
            ...
            'libmemcached' => 'Lbb\Session\Adapter\Libmemcached',
